...
// ── PAYWALL ─────────────────────────────────────────────────────
function Paywall({ course, onPay, onClose }) {
  const [step, setStep] = useState("info");
  const ref = "CRS" + Math.random().toString(36).substring(2, 8).toUpperCase();
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "flex-end", justifyContent: "center", zIndex: 200 }}>
      <div style={{ background: C.surface, borderRadius: "24px 24px 0 0", padding: "32px 24px 44px", width: "100%", maxWidth: 480, boxShadow: "0 -8px 48px rgba(0,0,0,0.25)" }}>
        {step === "info" && <>
          ...
          <button onClick={() => { setStep("processing"); setTimeout(() => setStep("success"), 2500); }}
        ...
        {step === "processing" && (
          <div style={{ textAlign: "center", padding: "50px 0" }}>
            <div style={{ width: 52, height: 52, border: `4px solid ${C.accentLight}`, borderTop: `4px solid ${C.accent}`, borderRadius: "50%", animation: "spin 0.9s linear infinite", margin: "0 auto 20px" }} />
            <div style={{ fontWeight: 700, fontSize: 16, color: C.text }}>Processing payment...</div>
            <div style={{ fontSize: 13, color: C.muted, marginTop: 6 }}>Ref: {ref}</div>
        ...
        {step === "success" && (
          <div style={{ textAlign: "center", padding: "24px 0" }}>
            ...
            <div style={{ color: C.muted, fontSize: 14, marginBottom: 4 }}>₦1,000 · Ref: {ref}</div>
...
export default function Coursia() {
    ...
    if (s.type === "name") return (
      <div style={{ minHeight: "100vh", background: `linear-gradient(135deg, ${C.primary} 0%, #1e0a4e 100%)`, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
        <div style={{ background: C.surface, borderRadius: 24, padding: "36px 28px", maxWidth: 420, width: "100%" }}>
          ...
          <input value={nameInput} onChange={e => setNameInput(e.target.value)} onKeyDown={e => e.key === "Enter" && nameInput.trim() && handleNameSubmit()} placeholder="Your first name"
            style={{ width: "100%", border: `1.5px solid ${C.border}`, borderRadius: 12, padding: "14px 16px", fontSize: 16, outline: "none", boxSizing: "border-box", fontFamily: "inherit", color: C.text, marginBottom: 14 }} autoFocus />
          <button onClick={handleNameSubmit} disabled={!nameInput.trim()}
            style={{ width: "100%", background: nameInput.trim() ? C.accent : C.border, color: nameInput.trim() ? C.primary : C.muted, border: "none", borderRadius: 12, padding: "15px 0", fontWeight: 900, fontSize: 16, cursor: nameInput.trim() ? "pointer" : "default" }}>
            Build My Plan →
          </button>
        </div>
      </div>
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "system-ui, -apple-system, sans-serif" }}>
      {toast && <Toast msg={toast} onClose={() => setToast(null)} />}
      {certCourse && <Certificate course={certCourse} userName={userName} onClose={() => setCertCourse(null)} />}
      {showPaywall && <Paywall course={showPaywall} onPay={handlePaySuccess} onClose={() => setShowPaywall(null)} />}
